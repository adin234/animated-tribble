    ...
    var data = {},
        ...
        start = function () {
            ...
            mysql.open(config.mysql)
                .query(
                    'SELECT user.*, refresh.*, youtube.field_value youtube \
                ...
                if (item.streams) {
                    item.streams.items.forEach(function (iitem) {
                        ...
                        if (!req.query.lanparty || (
                                req.query.lanparty && (topush.youtube.snippet
                                    .description.reIndexOf(lanparty_check) || topush.youtube.snippet
    ...
    var data = {},
	    ...
	    result.livestream &&
                result.livestream.forEach(function (stream) {
                    ...
                    data[stream.media_name]['hitbox'] = {livestream: [stream]};
...
exports.get_streamers = function (req, res, next) {
    var data = {},
        ...
        start = function () {
            ...
            if (req.query.lanparty) {
                join =
                    'INNER JOIN xf_user_group_relation ON \
            ...
            mysql.open(config.mysql)
                .query(
					...
					' +
                    join + ' WHERE field_id = ? AND field_value != ""' + where + '  ORDER BY xf_user.user_id ASC', [
                        'twitchStreams'
                    ],
                    format_buffer
                )
        ...
        format_buffer = function (err, result) {
            ...
            result.forEach(function (item, i) {
                ...
                item.secondary_group_ids = new Buffer(item.secondary_group_ids, 'binary')
                    .toString();
                item.field_id = new Buffer(item.field_id, 'binary')
                    .toString();
            });
...
exports.get_streamers_data = function (req, res, next) {
    var data = {},
        ...
        get_videos = function (err, result) {
            var searchString = typeof req.query.search != 'undefined' ? regexEscape(req.query.search) : '';
