    ...
    var data = {},
        ...
        start = function() {
            ...
            mysql.open(config.mysql)
                .query(
                    'SELECT option_value FROM EWRporta_options WHERE option_id = ?', ['recentnews_forum'],
                    get_feature_list
                ).end();
        },
        ...
        get_slider = function(err, result) {
            ...
            mysql.open(config.mysql)
                .query(
					...
					LIMIT " + $options['recentfeatures_limit'], [$options['recentfeatures_filename'], date, date],
        ...
        get_latest_videos = function(err, result) {
            ...
            mysql.open(config.mysql)
                .query('select count(comment_id) as comments, video_id from anytv_comments' + ' where video_id in(\'' + ids.join('\',\'') + '\')' + ' group by video_id', [],
                    function(e, rlt) {
                        if (e) {
                            console.log(e);
                            return next(e)
